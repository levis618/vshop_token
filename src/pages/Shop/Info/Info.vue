...
export default {
  ...
  methods: {
    initScroll () {
      this.$nextTick(() => {
        if (!this.infoScroll) {
          this.infoScroll = new BScroll('.shop-info', { click: true })
        } else {
          this.infoScroll.refresh()
        }

        let ul = this.$refs.picsUI
        let liWidth = ul.getElementsByTagName('li')[0].clientWidth
        let space = 6
        let liCount = this.info.pics.length
        let width = (liWidth + space) * liCount - space
        ul.style.width = width + 'px'

        if (!this.picScroll) {
          this.picScroll = new BScroll('.pic-wrapper', { click: true, scrollX: true })
        } else {
